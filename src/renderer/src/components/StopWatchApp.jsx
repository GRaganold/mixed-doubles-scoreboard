...
const Stopwatch = () => {
	...
	useEffect(() => {
		...
		const handleKeyDown = event => {
			if (event.key === "Enter") {
				...
				startAndStop()
			} else if (event.key === "6" || event.key === "Digit6" || event.key === "Numpad6" || event.key === "ArrowRight") {
	...
	return (
		<VStack className="stopwatch-container" border="1px dotted black" paddingLeft={1} paddingRight={1}>
			...
			<HStack className="stopwatch-buttons" gap={1}>
				<Button className="stopwatch-button" bg={isRunning === true ? "#00aa86" : "#fd8050"} onClick={startAndStop} size={"sm"}>
