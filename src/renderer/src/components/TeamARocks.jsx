  ...
  useEffect(() => {
    if (hammerState === 'A' && (teamARockCountState + teamBRockCountState) % 2 !== 0) {
      const intervalId = setInterval(() => {
        setHighlightedRockWithHammer((prev) => (prev === -1 ? teamARockCountState - 1 : -1))
      }, flashInterval)
    ...
    <>
      {Array.from({ length: teamARockCountState }, (_, index) => (
        <Box
          key={index}
          borderLeft={
            gameRockCountState === 5
              ? index !== 0 && index !== 3 && index !== 2
                ? '2px solid gray'
                : undefined
              : index !== 0 && index !== 1 && index !== 3 && index !== 5 && index !== 7
                ? '2px solid gray'
                : undefined
          }
        >
          <Flex borderRadius={0} m="1px">
            <CircleIcon
              ...
              stroke={
                index === highlightedRockWithHammer ? 'transparent' : 'url(#black-gray-black)'
              }
            />
          </Flex>
        </Box>
