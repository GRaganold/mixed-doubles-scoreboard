...
function WithHammerTeamB({ teamBRockCountState, teamBColor, teamARockCountState, hammerState }) {
	...
	return (
		<>
			{Array.from({ length: teamBRockCountState }, (_, index) => (
				<Box key={index} borderLeft={index !== 0 && index !== 3 && index !== 2 ? "2px solid gray" : undefined}>
...
function WithoutHammerTeamB({ teamBRockCountState, teamBColor, teamARockCountState, hammerState }) {
	const [highlightedRockWithHammer, setHighlightedRockWithHammer] = useState(-1)
...
export default function TeamBRocks({ teamBRockCountState, teamBColor, teamARockCountState, teamAColor, hammerState }) {
	return (
		<>
			<Flex
				direction={"row"}
